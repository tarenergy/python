...
import subprocess # Für wmctrl Aufruf unter Linux
import sys # Für sys.platform benötigt
...
if __name__ == "__main__":
    ...
    # --- Versuch, Pygame-Fenster auch "topmost" zu machen (Plattformabhängig) ---
    # Ziel: Verhindern, dass es hinter anderen Apps verschwindet, wenn Tkinter Fokus hat.
    # Tkinter wird später nochmals topmost gesetzt und sollte darüber liegen.
    try:
        # Warte kurz, damit das Fenster sicher vom Window Manager erfasst wurde
        pygame.time.wait(100) # 100ms Pause

        wm_info = pygame.display.get_wm_info()
        hwnd = None # Keep variable for potential future cross-platform use
        platform_msg = ""

        # Windows (Beibehalten für Vollständigkeit)
        if sys.platform == 'win32' and 'window' in wm_info: # Prüfe auf 'win32' statt os.name
            hwnd = wm_info['window']
            try:
                import win32gui
                import win32con
                win32gui.SetWindowPos(hwnd, win32con.HWND_TOPMOST, 0, 0, 0, 0, win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
                platform_msg = "Pygame-Fenster auf topmost gesetzt (via win32gui)."
            except ImportError:
                platform_msg = "Pygame-Fenster NICHT topmost (win32gui nicht installiert)."
            except Exception as e_win:
                platform_msg = f"Fehler beim Setzen von Pygame-Fenster topmost (Windows): {e_win}"

        # Linux (X11) mit wmctrl
        elif sys.platform.startswith('linux') and 'window' in wm_info:
            xid = wm_info['window']
            try:
                # Kommando: wmctrl -i -r <XID> -b add,above
                result = subprocess.run(['wmctrl', '-i', '-r', str(xid), '-b', 'add,above'],
                                        check=True, capture_output=True, text=True, timeout=2)
                platform_msg = f"Pygame-Fenster auf 'always on top' gesetzt (via wmctrl)."
            except FileNotFoundError:
                platform_msg = "Pygame-Fenster NICHT 'always on top' (wmctrl nicht gefunden/installiert)."
                print("Hinweis: Installiere 'wmctrl' mit 'sudo apt update && sudo apt install wmctrl', um das Pygame-Fenster im Vordergrund zu halten.")
            except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e_wmctrl:
                platform_msg = f"Fehler beim Ausführen von wmctrl: {e_wmctrl}"
            except Exception as e_linux:
                platform_msg = f"Fehler beim Setzen von Pygame-Fenster 'always on top' (Linux): {e_linux}"

        # macOS (Platzhalter - erfordert evtl. pyobjc)
        elif sys.platform == 'darwin' and 'window' in wm_info:
             platform_msg = "Pygame-Fenster NICHT topmost (macOS/Cocoa Implementierung nötig)."

        if platform_msg: print(platform_msg)

    except Exception as e_wm:
        print(f"Fehler beim Abrufen/Verwenden von WM-Info: {e_wm}")
    # --- Ende Topmost-Versuch für Pygame ---
