...
class AudioEngine:
    def __init__(self):
        ...
        self.active_notes = {}
        self.master_channel = pygame.mixer.Channel(0)
        self.master_channel.set_volume(0.8)
    ...
    def generate_sound(self, frequency):
        ...
        wave += 0.1 * amplitude * np.sin(2 * np.pi * frequency * 3 * t)  # 2. Oberton
        wave += 0.05 * amplitude * np.sin(2 * np.pi * frequency * 4 * t)  # 3. Oberton
        
        # Attack-Decay-Sustain-Release Hüllkurve
        attack_time = 0.01  # 10ms
        decay_time = 0.1    # 100ms
        release_time = 0.3  # 300ms
                ...
                for key in black_keys:
                    if key['rect'].collidepoint(mouse_pos):
                        ...
                        if key['active']:
                            ...
                            audio_engine.stop_note(key['note'])
