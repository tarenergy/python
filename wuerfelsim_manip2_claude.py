...
class DiceSimulator:
    ...
    def create_widgets(self):
        ...
        dice_spinbox = ttk.Spinbox(control_frame, from_=1, to=1000, textvariable=self.dice_var, width=5)
        ...
        rolls_spinbox = ttk.Spinbox(control_frame, from_=100, to=1000000, textvariable=self.rolls_var, width=8)
    ...
    def calculate_theoretical_probabilities(self, num_dice):
        ...
        # Bei sehr vielen Würfeln können wir eine Normalverteilungsannäherung verwenden
        if num_dice > 100:
            # Für sehr viele Würfel: Normalverteilungsannäherung
            # Mittelwert = num_dice * 3.5 (Erwartungswert eines Würfels)
            # Standardabweichung = sqrt(num_dice * 35/12) (Varianz eines Würfels = 35/12)
            mean = num_dice * 3.5
            std_dev = np.sqrt(num_dice * 35/12)
            
            # Berechne Wahrscheinlichkeiten für alle möglichen Summen mit Normalverteilung
            probs = []
            for s in possible_sums:
                # Kontinuitätskorrektur für diskrete Werte
                p = (np.exp(-0.5 * ((s - 0.5 - mean) / std_dev)**2) - 
                     np.exp(-0.5 * ((s + 0.5 - mean) / std_dev)**2)) / np.sqrt(2 * np.pi) / std_dev
                probs.append(p)
            
            # Normalisiere die Wahrscheinlichkeiten, damit sie sich zu 1 summieren
            total = sum(probs)
            theoretical_probs = np.array([p / total for p in probs])
        else:
            # Für weniger Würfel: Exakte Berechnung mit rekursivem Ansatz
            # Beginne mit einer Wahrscheinlichkeitsverteilung für einen Würfel
    ...
    def calculate_theoretical_probabilities_biased(self, num_dice, face_probs):
        ...
        # Bei sehr vielen Würfeln: Normalverteilungsannäherung
        if num_dice > 100:
            # Berechne Erwartungswert und Varianz eines manipulierten Würfels
            expected_value = sum([(i+1) * face_probs[i] for i in range(6)])
            variance = sum([(i+1)**2 * face_probs[i] for i in range(6)]) - expected_value**2
            
            # Mittelwert und Standardabweichung für die Summe von num_dice Würfeln
            mean = num_dice * expected_value
            std_dev = np.sqrt(num_dice * variance)
            ...
            # Für weniger Würfel: Exakte Berechnung
    ...
    def plot_results(self, possible_sums, experimental_counts, theoretical_counts_fair, 
        ...
        # Bei sehr vielen möglichen Summen (z.B. bei vielen Würfeln) gruppieren wir die Daten
        num_bins = min(50, len(possible_sums))  # Maximal 50 Balken anzeigen
        
        if len(possible_sums) > num_bins:
            # Gruppiere die Daten in Bins
            bin_size = len(possible_sums) // num_bins
            grouped_sums = []
            grouped_exp = []
            grouped_fair = []
            grouped_biased = []
            
            for i in range(0, len(possible_sums), bin_size):
                end_idx = min(i + bin_size, len(possible_sums))
                sum_range = f"{possible_sums[i]}-{possible_sums[end_idx-1]}"
                grouped_sums.append(sum_range)
                grouped_exp.append(np.sum(experimental_counts[i:end_idx]))
                grouped_fair.append(np.sum(theoretical_counts_fair[i:end_idx]))
                grouped_biased.append(np.sum(theoretical_counts_biased[i:end_idx]))
            
            # Ersetze die ursprünglichen Daten durch die gruppierten Daten
            display_sums = grouped_sums
            display_exp = np.array(grouped_exp)
            display_fair = np.array(grouped_fair)
            display_biased = np.array(grouped_biased)
        else:
            # Für weniger Summen: direkt anzeigen
            display_sums = possible_sums
            display_exp = experimental_counts
            display_fair = theoretical_counts_fair
            display_biased = theoretical_counts_biased
        
        # Balkenbreite berechnen
        bar_width = 0.3
        index = np.arange(len(display_sums))
        
        # Experimentelle Häufigkeiten plotten
        exp_bars = self.ax.bar(index - bar_width, display_exp, bar_width, 
                              alpha=0.7, color='blue', label='Experimentell')
        
        # Theoretische Häufigkeiten für faire Würfel plotten
        fair_bars = self.ax.bar(index, display_fair, bar_width, 
        ...
        if six_bias > 0 and not np.array_equal(theoretical_counts_fair, theoretical_counts_biased):
            biased_bars = self.ax.bar(index + bar_width, display_biased, bar_width, 
        ...
        if len(display_sums) > 20:
            # Bei vielen Werten nur jeden n-ten Wert anzeigen
            step = max(1, len(display_sums) // 20)
            self.ax.set_xticks(index[::step])
            self.ax.set_xticklabels(display_sums[::step], rotation=45, ha='right')
        else:
            self.ax.set_xticks(index)
            self.ax.set_xticklabels(display_sums)
