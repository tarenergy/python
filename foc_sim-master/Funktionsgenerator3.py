...
def generate_waveform(wave_type, freqs, sample_rate):
    ...
    elif wave_type == "Gespenstergeräusche":
        wave = generate_ghost_sounds(len(freqs), sample_rate, freqs.min(), freqs.max())
    else:
...
def generate_ghost_sounds(samples, sample_rate, min_freq, max_freq):
    """Erzeugt gespenstische Geräusche durch moduliertes Rauschen."""
    noise = np.random.randn(samples) * np.sin(np.linspace(0, np.pi * 2, samples) * np.random.uniform(0.1, 5))
    fft_noise = np.fft.rfft(noise)
    freqs = np.fft.rfftfreq(samples, 1 / sample_rate)
    mask = (freqs >= min_freq) & (freqs <= max_freq)
    fft_noise[~mask] = 0  # Frequenzen außerhalb des Bereichs entfernen
    ghost_noise = np.fft.irfft(fft_noise)
    ghost_noise /= np.max(np.abs(ghost_noise))  # Normalisieren
    return ghost_noise * 0.5
...
def generate_wobble_wave(start_freq, end_freq, duration, wave_type, sample_rate=44100):
    ...
    wobble_freq = 0.5 * (1 - np.cos(2 * np.pi * t))  # Korrektur für gleichmäßiges Wobbeln
    freqs = start_freq + (end_freq - start_freq) * wobble_freq
...
tk.OptionMenu(root, wave_type_var, "Sinus", "Rechteck", "Dreieck", "Impuls", "Rosa Rauschen", "Gespenstergeräusche").pack()
