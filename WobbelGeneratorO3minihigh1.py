...
def generate_oscillator_block(n, t0):
    ...
    with params_lock:
        ...
        wobble_speed = params['wobble_speed']
        volume = params['volume']
        waveform = params['waveform']
    t = np.arange(n) / fs + t0
    # LFO: oscillates smoothly between 0 and 1
    lfo = 0.5 * (1 + np.sin(2 * np.pi * wobble_speed * t))
...
def update_start_freq(val):
    with params_lock:
        params['start_freq'] = float(start_freq_scale.get())

def update_end_freq(val):
    with params_lock:
        params['end_freq'] = float(end_freq_scale.get())

def update_wobble_speed(val):
    with params_lock:
        params['wobble_speed'] = float(wobble_speed_scale.get())

def update_volume(val):
    with params_lock:
        params['volume'] = float(volume_scale.get())
...
tk.Label(root, text="Startfrequenz (Hz)").grid(row=1, column=0, sticky="w")
start_freq_scale = tk.Scale(root, from_=20, to=2000, orient=tk.HORIZONTAL, command=update_start_freq)
start_freq_scale.set(200)
start_freq_scale.grid(row=1, column=1)

tk.Label(root, text="Endfrequenz (Hz)").grid(row=2, column=0, sticky="w")
end_freq_scale = tk.Scale(root, from_=20, to=10000, orient=tk.HORIZONTAL, command=update_end_freq)
end_freq_scale.set(800)
end_freq_scale.grid(row=2, column=1)

tk.Label(root, text="Wobbelgeschwindigkeit (Hz)").grid(row=3, column=0, sticky="w")
wobble_speed_scale = tk.Scale(root, from_=0.1, to=10, resolution=0.1, orient=tk.HORIZONTAL, command=update_wobble_speed)
wobble_speed_scale.set(1.0)
wobble_speed_scale.grid(row=3, column=1)

tk.Label(root, text="Lautstärke").grid(row=4, column=0, sticky="w")
volume_scale = tk.Scale(root, from_=0.0, to=1.0, resolution=0.01, orient=tk.HORIZONTAL, command=update_volume)
volume_scale.set(0.5)
volume_scale.grid(row=4, column=1)
