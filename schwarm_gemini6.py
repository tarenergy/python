...
class Boid:
    def __init__(self, x, y, color, params):
        ...
        # Verwende korrekten Key (ohne Prefix für Boid-Params)
        ...
        self.color = color
        self.params = params # Referenz auf Schwarm-Parameter (tk.Vars)
        self.size = 6
    ...
    def seek(self, target):
        # Verwendet korrekte Keys für Boids
        ...
        for other in boids:
            ...
            if self != other and 0 < dist_sq < vis_range * vis_range and other.color == self.color:
                 if other.velocity and other.velocity.length_squared() > 0:
                    steering += other.velocity; total += 1
        if total > 0:
            steering /= total
            if steering.length() > 0: steering = steering.normalize() * max_speed
            steer = steering - self.velocity
    ...
    def cohere(self, boids):
        ...
        # Verwendet korrekten Key für Boids
        vis_range = self.params['visual_range'].get()
        for other in boids:
            try: dist_sq = self.position.distance_squared_to(other.position)
            except OverflowError: dist_sq = float('inf')
...
class Shark:
    def __init__(self, x, y, params): # Nimmt params entgegen
        ...
        # Initialisiere eat_radius_sq (wird in update-Schleife aktualisiert)
        try:
            # KORREKTUR: Verwende 'shark_eat_radius' Key
            current_eat_radius = self.params['shark_eat_radius'].get()
            self.eat_radius_sq = current_eat_radius * current_eat_radius
        except KeyError:
             print("FEHLER: 'shark_eat_radius' nicht in params gefunden! Verwende Fallback.")
             eat_radius_fallback = 13.0
             self.eat_radius_sq = eat_radius_fallback * eat_radius_fallback
    ...
    def seek(self, target_pos):
        # KORREKTUR: Verwende 'shark_' Prefixe
        max_speed = self.params['shark_max_speed'].get()
    ...
    def hunt(self, all_boids):
        closest_boid = None
        # KORREKTUR: Verwende 'shark_' Prefix
        perception_radius = self.params['shark_perception_radius'].get()
        min_dist_sq = perception_radius * perception_radius
        for boid in all_boids:
            try:
                dist_sq = self.position.distance_squared_to(boid.position)
                if dist_sq < min_dist_sq:
                    min_dist_sq = dist_sq; closest_boid = boid
    ...
    def wander(self):
        ...
        wander_force = wander_target_direction * max_force * 0.5
...
# Slider Definitionen erweitert für Hai (mit Prefixen)
slider_definitions = {
    # Boids
    ...
    'max_speed': ("Max. Tempo (Fisch)", 1.0, 10.0, 0.1), # Key für Boids
    'max_force': ("Max. Lenkkraft (Fisch)", 0.01, 1.0, 0.01), # Key für Boids
    # Hai
    'shark_max_speed': ("Max. Tempo (Hai)", 1.0, 15.0, 0.1), # Eigener Key
    'shark_max_force': ("Max. Lenkkraft (Hai)", 0.01, 1.5, 0.01), # Eigener Key
    'shark_perception_radius': ("Sichtweite (Hai)", 50.0, 400.0, 1.0), # Eigener Key
    'shark_eat_radius': ("Fressradius (Hai)", 5.0, 50.0, 0.5), # Eigener Key
}

# Standardwerte getrennt für Boids und Hai
...
default_values_shark = {
    'shark_max_speed': 3.8,
    'shark_max_force': 0.35,
    'shark_perception_radius': 400.0,
...
def create_sliders(parent_frame, params_dict, default_values):
    """Erstellt Slider basierend auf den Einträgen in default_values."""
    row_index = 0
    for key in default_values: # Iteriere über die spezifischen Defaults für diesen Frame
...
# Setup Tkinter Controls Funktion (MODIFIZIERT)
def setup_tkinter_controls():
    root = tk.Tk()
    root.title("Boid Parameter")
    root.geometry("550x700") # BREITER
    ...
    frame_shark = ttk.Frame(notebook, padding="10") # << NEU
    notebook.add(frame_shark, text='Hai Parameter') # << NEU
    create_sliders(frame_shark, params_shark, default_values_shark) # << NEU

    return root, params_swarm1, params_swarm2, params_shark # << Angepasst


# --- Hauptteil ---
if __name__ == "__main__":
    # Hole alle Parameter-Dictionaries
    root, params1, params2, params_shark = setup_tkinter_controls() # << Angepasst
    pygame.init()
    pygame.font.init()
    ...
    try:
        ...
        # Übergebe params_shark an den Hai Konstruktor
        shark = Shark(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, params_shark) # << Angepasst
    ...
    def simulation_update():
        # Füge params_shark zur globalen Liste hinzu
        ...
        eat_radius_sq = shark.eat_radius_sq # Standardwert
        try:
             # KORREKTUR: Verwende 'shark_eat_radius'
             current_eat_radius = params_shark['shark_eat_radius'].get()
             eat_radius_sq = current_eat_radius * current_eat_radius
             shark.eat_radius_sq = eat_radius_sq # Aktualisiere internen Wert für nächste Prüfungen
        except tk.TclError: pass
        except KeyError: pass # Falls Key noch nicht da ist (sollte nicht passieren)
        ...
        for boid in boids_to_check:
            try:
                ...
                # Verwende den oben aktualisierten eat_radius_sq
