...
class Shark:
    def __init__(self, x, y):
        ...
        # Korrektur: Fressradius muss auch quadriert werden für Vergleich
        self.eat_radius_sq = (self.size * 1.1) * (self.size * 1.1)
    ...
    def hunt(self, all_boids):
        ...
        for boid in all_boids:
            ...
            except AttributeError: continue # Falls boid kein gültiges Objekt mehr ist
        if closest_boid:
...
# --- Globale Variablen und Definitionen (Definitionen vor Funktionen) ---
running = True
screen = None
...
def create_sliders(parent_frame, params_dict, default_values):
    ...
    for key in slider_definitions:
        ...
        if key == 'boid_count': # Nur hier das zusätzliche Label
            ...
            params_dict['boid_count_actual_label'] = actual_count_label # Referenz speichern
...
if __name__ == "__main__":
    ...
    try:
        ...
        all_boids = swarm1.boids + swarm2.boids # Globale Liste für alle Boids
    ...
    def simulation_update():
        # Deklariere alle globalen Variablen, die in der Funktion modifiziert werden könnten
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.VIDEORESIZE and not (screen.get_flags() & pygame.FULLSCREEN):
                 global SCREEN_WIDTH, SCREEN_HEIGHT # Aktualisiere globale Größe bei Resize
                 SCREEN_WIDTH, SCREEN_HEIGHT = event.w, event.h
                 try: screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.RESIZABLE | pygame.DOUBLEBUF)
                 except pygame.error as e: print(f"Fehler Resize: {e}")

        # === Boid Anzahl anpassen (durch Slider Zielwert) ===
        ...
        # Wenn Slider die Anzahl geändert hat, baue die globale Liste neu auf
        if count_changed_by_slider:
            all_boids = swarm1.boids + swarm2.boids

        # === Hai frisst Fische ===
        eaten_this_frame = []
        # Erstelle eine Kopie der Liste, über die iteriert wird, da das Original modifiziert wird
        boids_to_check = list(all_boids)
        # eat_radius_sq wurde im Shark Konstruktor hinzugefügt

        for boid in boids_to_check:
            try:
                dist_sq = shark.position.distance_squared_to(boid.position)
                # Verwende den vorberechneten quadrierten Radius
                if dist_sq < shark.eat_radius_sq:
                    # Stelle sicher, dass derselbe Boid nicht mehrfach hinzugefügt wird
                    # (könnte passieren, wenn er schon entfernt wurde, aber noch in boids_to_check ist)
                    if boid in all_boids and boid not in eaten_this_frame:
                        eaten_this_frame.append(boid)
            except OverflowError: continue
            except AttributeError: continue # Falls boid Objekt ungültig ist

        # Entferne gefressene Boids *nach* der Überprüfungsschleife
        if eaten_this_frame:
            # print(f"Hai hat {len(eaten_this_frame)} Fisch(e) gefressen!") # DEBUG
            boids_removed_count = 0
            for eaten_boid in eaten_this_frame:
                removed_from_any = False
                # Entferne aus globaler Liste (mit Fehlerprüfung)
                try:
                    if eaten_boid in all_boids:
                        all_boids.remove(eaten_boid)
                        # print(f"  Removed {id(eaten_boid)} from all_boids. New len: {len(all_boids)}") # DEBUG
                        removed_from_any = True
                except ValueError:
                    # print(f"  Attempted to remove {id(eaten_boid)} from all_boids, but not found.") # DEBUG
                    pass

                # Entferne aus spezifischer Schwarm-Liste (mit Fehlerprüfung)
                try:
                    if eaten_boid.color == BLUE and eaten_boid in swarm1.boids:
                        swarm1.boids.remove(eaten_boid)
                        # print(f"  Removed {id(eaten_boid)} from swarm1.boids. New len: {len(swarm1.boids)}") # DEBUG
                        removed_from_any = True
                    elif eaten_boid.color == RED and eaten_boid in swarm2.boids:
                        swarm2.boids.remove(eaten_boid)
                        # print(f"  Removed {id(eaten_boid)} from swarm2.boids. New len: {len(swarm2.boids)}") # DEBUG
                        removed_from_any = True
                except ValueError:
                     # print(f"  Attempted to remove {id(eaten_boid)} from swarm list, but not found.") # DEBUG
                     pass

                if removed_from_any:
                     boids_removed_count += 1
            # print(f"  Actually removed: {boids_removed_count}. Verbleibend all: {len(all_boids)}, S1: {len(swarm1.boids)}, S2: {len(swarm2.boids)}") # DEBUG
        ...
        try:
            ...
            if len(all_boids) > 0:
                 swarm1.run(screen, all_boids) # Übergibt all_boids für Interaktion, intern wird self.boids verwendet
        ...
        try:
            if root.winfo_exists():
                # Update der "Aktuell:" Labels
                actual_count1 = len(swarm1.boids) # Länge der internen Schwarm-Liste
