...
class AudioGenerator:
    def __init__(self):
        ...
        self.chunk = 1024
        self.running = False
        self.phase = 0.0  # Für mono-Wellenformen
        self.phase_start = 0.0  # Für Doppelton, Komponente 1
        self.phase_end = 0.0    # Für Doppelton, Komponente 2
        self.start_time = time.perf_counter()
        self.param_lock = threading.Lock()
        self.params = {
            'waveform': 'Sinus',        # Options: Sinus, Rechteck, Dreieck, Weiss, Rosa_BP, Rosa_Voss, Rosa_iFFT, Doppelton
    ...
    def callback(self, in_data, frame_count, time_info, status):
        ...
        if params['waveform'] == 'Doppelton':
            ...
            for i in range(frame_count):
                phase_inc_start = 2 * math.pi * params['freq_start'] * dt
                phase_inc_end = 2 * math.pi * params['freq_end'] * dt
                self.phase_start += phase_inc_start
                self.phase_end += phase_inc_end
                if self.phase_start > 2 * math.pi:
                    self.phase_start -= 2 * math.pi
                if self.phase_end > 2 * math.pi:
                    self.phase_end -= 2 * math.pi
                sample_start = math.sin(self.phase_start)
                sample_end = math.sin(self.phase_end)
                sample_mix = (sample_start + sample_end) / 2.0
                samples_left[i] = sample_mix * params['left_volume']
                samples_right[i] = ((math.sin(self.phase_start + phase_shift_rad) + math.sin(self.phase_end + phase_shift_rad)) / 2.0) * params['right_volume']
            interleaved = np.empty(frame_count * 2, dtype=np.float32)
            interleaved[0::2] = samples_left
            interleaved[1::2] = samples_right
            return (interleaved.tobytes(), pyaudio.paContinue if self.running else pyaudio.paComplete)
        ...
        for i in range(frame_count):
            ...
            # Berechne die Frequenz basierend auf dem Modus
...
class App:
    ...
    def create_widgets(self):
        ...
        self.waveform_menu = ttk.Combobox(self.frame, textvariable=self.waveform_var, 
                                          values=["Sinus", "Rechteck", "Dreieck", "Weiss", "Rosa_BP", "Rosa_Voss", "Rosa_iFFT", "Doppelton"], 
                                          state="readonly")
