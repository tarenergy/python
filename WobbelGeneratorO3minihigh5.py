...
params = {
    ...
    'phase_shift': 0.0,      # in degrees (für Sinus)
    'wobble_interp_log': True,  # logarithmische Interpolation für Wobbelung
    'wobble_enabled': True      # Wobbeln ein-/ausschalten
}
...
def generate_oscillator_stereo_block(n, t0):
    ...
    with params_lock:
        ...
        wobble_enabled = params['wobble_enabled']
    t = np.arange(n) / fs + t0
    if wobble_enabled:
...
def update_wobble_enabled():
    with params_lock:
        params['wobble_enabled'] = bool(wobble_enabled_var.get())
...
wobble_enabled_var = tk.IntVar()
wobble_enabled_var.set(1)
wobble_enabled_check = tk.Checkbutton(root, text="Wobbeln aktivieren", variable=wobble_enabled_var, command=update_wobble_enabled)
wobble_enabled_check.grid(row=4, column=0, columnspan=3, pady=5)

interp_var = tk.IntVar()
interp_var.set(1)  # 1 für logarithmische Interpolation
...
interp_check.grid(row=5, column=0, columnspan=3, pady=5)

tk.Label(root, text="Lautstärke links").grid(row=6, column=0, sticky="w")
...
volume_left_scale.grid(row=6, column=1, columnspan=2)

tk.Label(root, text="Lautstärke rechts").grid(row=7, column=0, sticky="w")
...
volume_right_scale.grid(row=7, column=1, columnspan=2)

tk.Label(root, text="Phasenverschiebung (Sinus) (-180° bis 180°)").grid(row=8, column=0, sticky="w")
...
phase_shift_scale.grid(row=8, column=1, columnspan=2)

start_button = tk.Button(root, text="Start", command=start_audio)
start_button.grid(row=9, column=0, pady=5)
stop_button = tk.Button(root, text="Stop", command=stop_audio)
stop_button.grid(row=9, column=1, pady=5)
